	...
	while(pMgr->nRunning)
	{
		...
		else
		{
			if(FD_ISSET(pMgr->nRtspSockFd, &nFdSet))
			{
				...
				sockfd = accept(pMgr->nRtspSockFd, (struct sockaddr *)&addr, &nLen);
				if(sockfd < 0)
				{
...
RtspHandle RtspInit(SRtspInitAttr *pInitAttr)
{
	...
	//printf("Enter %s()......\n", __FUNCTION__);
	//dbgprint(LOG_LEVEL_RTSP,"Enter %s()......\n", __FUNCTION__);
  ...
  pMgr->nRtcpSockFd = socket(AF_INET, SOCK_DGRAM, 0);
	...
	//printf("Rtsp server is ok! \nPrint rtsp://xxx.xxx.xxx.xxx/live0.264 in the rtsp client.\n");
	//dbgprint(LOG_LEVEL_RTSP,"Rtsp server is ok! \n");
	//dbgprint(LOG_LEVEL_RTSP,"Print rtsp://xxx.xxx.xxx.xxx/live0.264 in the rtsp client.\n");
