...
void udpInit(char* addrStr, HI_U32 port)
{
    int s, len;
...
HI_S32 SAMPLE_COMM_VENC_UDP(VENC_STREAM_S *stStream)
{
        ...
        if(nRecordFlag){
            for(j = 0;j < stStream->u32PackCount;j++){
                ...
                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_SPS){
                    ...
                    ret = sendto(server, pData, len, 0, (struct sockaddr *) &addr, addr_len);

                    //写sps
                    printf("count %d,j %d,ret %d Write sps =================\n",stStream->u32PackCount,j,ret);    
                ...
                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_PPS){
                    ...
                    //写pps                                  
                    //printf("count %d,j %d,ret %d Write pps -------------------\n",stStream->u32PackCount,j,ret);                                      
                ...
                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_ISLICE){
                    ...
                    //写I frame                                  
                    //printf("count %d,j %d,ret %d I frame -------------------\n",stStream->u32PackCount,j,ret);                                      
                        
                    continue;
                }

                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_PSLICE){   
                    ret = sendto(server, pData, len, 0, (struct sockaddr *) &addr, addr_len);

                    //写P frame                                 
                    //printf("count %d,j %d,ret %d P frame -------------------\n",stStream->u32PackCount,j,ret);                                   
                        
                    continue;
                }

                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_BSLICE){   
                    ret = sendto(server, pData, len, 0, (struct sockaddr *) &addr, addr_len);

                    //写B frame                                 
                    //printf("count %d,j %d,ret %d B frame -------------------\n",stStream->u32PackCount,j,ret);                                      
                        
                    continue;
                }

                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_IDRSLICE){   
                    ret = sendto(server, pData, len, 0, (struct sockaddr *) &addr, addr_len);

                    //写IDR frame                                 
                    //printf("count %d,j %d,ret %d IDR frame -------------------\n",stStream->u32PackCount,j,ret);                                      
                        
                   
                    continue;
                }

                if(stStream->pstPack[j].DataType.enH264EType == H264E_NALU_SEI){   
                    ret = sendto(server, pData, len, 0, (struct sockaddr *) &addr, addr_len);

                    //写SEI frame                                 
                    //printf("count %d,j %d,ret %d SEI frame -------------------\n",stStream->u32PackCount,j,ret);                                      
