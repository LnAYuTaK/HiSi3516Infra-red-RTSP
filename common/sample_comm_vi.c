...
// VI_DEV_ATTR_EX_S DEV_ATTR_BT1120_BASE_EX =
// {
...
HI_S32 SAMPLE_COMM_VI_SetParam(SAMPLE_VI_CONFIG_S *pstViConfig)
{
    ...
    for (i = 0; i < pstViConfig->s32WorkingViNum; i++)
    {
        s32ViNum  = pstViConfig->as32WorkingViId[i];
        pstViInfo = &pstViConfig->astViInfo[s32ViNum];
        ViPipe    = pstViInfo->stPipeInfo.aPipe[0];//0
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            if (HI_TRUE == pstViInfo->stPipeInfo.bIspBypass)
            {
                ...
                //stPipeAttr.enBitWidth = DATA_BITWIDTH_16;
            }
...
HI_S32 SAMPLE_COMM_VI_StartIsp(SAMPLE_VI_INFO_S *pstViInfo)
{
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        ...
        //lcc
        /*
        if(pstViInfo->stSnsInfo.enSnsType==BT1120_2M_30FPS_8BIT){
            continue;
        }
        */
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
...
HI_VOID *SAMPLE_COMM_VI_DumpRaw(HI_VOID *arg)
{
    ...
    //lcc
    /*
    VI_DUMP_ATTR_S vi_dump_attr_s;
    vi_dump_attr_s.bEnable = HI_TRUE;
    vi_dump_attr_s.u32Depth = 8;
    //vi_dump_attr_s.enDumpType = VI_DUMP_TYPE_YUV;
    vi_dump_attr_s.enDumpType = VI_DUMP_TYPE_RAW;
    s32Ret = HI_MPI_VI_SetPipeDumpAttr(ViPipe, &vi_dump_attr_s);
    ...
    */
    ...
    //s32Ret = HI_MPI_VPSS_GetChnFrame(0, 0, &stVideoFrame, 2000);
    ...
    //s32Ret = HI_MPI_VPSS_ReleaseChnFrame(0, 0, &stVideoFrame);
    ...
    while ((HI_TRUE == g_stViDumpRawThreadInfo.bDump) && (s32DumpCnt < s32Cnt))
    {
        ...
        s32Ret = SAMPLE_COMM_VI_SaveRaw(&stVideoFrame.stVFrame, 8, pfile);
