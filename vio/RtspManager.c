...
static int venc_start(VENC_CHN chn)
{
	...
	printf("g_sManager_VencCh0.venc_fd = %d", g_sManager_VencCh0.venc_fd);
...
static unsigned int venc_read(unsigned char *in_buf, unsigned int in_len, venc_stream_s *pout_stream, VENC_CHN chn, FILE *pFd)
{
	...
	// SRtspGroupAttr g_sManager = choice(chn);
...
static void *host_encoding(void *attr)
{
	...
	if (0 == venc_start(g_sManager->VencChn))
	{
		...
		printf("start encoder chn[%d] OK!!\n", i);
	}
	else
	{
		printf("start encoder chn[%d] failed!!\n", i);
	}
	...
	while (1)
	{
		...
		// printf("g_sManager->rtsp_toworking = %d,g_sManager->rtsp_isworking = %d\n",g_sManager->rtsp_toworking,g_sManager->rtsp_isworking);
		// if(g_sManager->rtsp_toworking==1 && g_sManager->rtsp_isworking==0 )
		//  if(1)
		//  {
		//  		printf("start venc\n");
		//  		if(0 == venc_start(g_sManager->VencChn))
		...
		//  		}
		//  }
		//  else if(g_sManager->rtsp_toworking==0 && g_sManager->rtsp_isworking==1)
		//  {
		//  		printf("stop venc\n");
		//  		if(0 == venc_stop(g_sManager->VencChn))
		//  		{
		//  			g_sManager->rtsp_isworking = 0;
		//  			printf("stop encoder chn[%d] OK!!\n", i);
		//  		}
		//  		else
		//  		{
		//  			printf("stop encoder chn[%d] failed!!\n", i);
		//  		}
		//  }else if(streamReqCntCur[0] > streamReqCntPast[0])
		//  {
		//  		printf("iray_venc_req_IDR(g_sManager->VencChn)\n");
		//  		iray_venc_req_IDR(g_sManager->VencChn);
		//  		printf("Apply a I frame, Automatically!!\n");
		//  }
		...
		if (g_sManager->rtsp_isworking)
		{
			...
			// printf("ret = %d,g_sManager->VencChn = %d\n",ret,g_sManager->VencChn);
			if (ret > 0)
			{
				// printf("ret = %d\n",ret);
...
static int RtspApplyKeyFrame(VENC_CHN chn)
{
	iray_venc_req_IDR(0);
	return 0;
}
...
int RtspInit_demo(void)
{
	...
	g_sManager_VencCh0.VpssChn = 1; // 2
	...
	{
		printf("can't hong create thread host encode!!\n");
		return -1;
	}
	// pthread_t pid2;//bai
	// if(0 != pthread_create(&pid2,NULL,(void* (*)(void*))host_encoding,(void *)(&g_sManager_VencCh1)))//开启通道1的rtsp
	//{
	//	printf("can't  bai create thread host encode!!\n");
	//	return -1;
	// }
